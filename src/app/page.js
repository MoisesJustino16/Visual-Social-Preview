...
import { 
  ...
  Share2, Copy, Layout, Plus, RefreshCw, Smartphone, ChevronLeft
} from 'lucide-react';
                    ...
                    <div>
                        ...
                        <div className="flex gap-3">
                            {['reels', 'tiktok', 'feed'].map(p => (
                                <button key={p} onClick={() => setPlatform(p)} className={`flex-1 py-3 capitalize rounded-xl text-xs font-bold border-2 transition-all ${platform === p ? 'border-black bg-black text-white' : 'border-gray-100 text-gray-400 hover:border-gray-300'}`}>{p}</button>
                            ))}
                        </div>
                    </div>
